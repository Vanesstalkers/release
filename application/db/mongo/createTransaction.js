async () => {
  class dataTransaction {
    ...
    async findOne(col, query, projection = {}) {
      ...
      if (!(doc || {})._id) {
        // объект может быть еще не создан
        ...
        if (this.data[doc._id]) {
          ...
          doc._meta = { _id: doc._id, col, initial: lib.utils.clone(doc) };
      ...
      if (parents) {
        ...
        parents
          ...
          .forEach((p) => {
            ...
            if ((!p_key && p_key !== false) || p_key === true)
              p_key = '__' + p_name;
            if (p_key) {
            ...
            // ! - дефолтная ссылка, false - ссылку не делаем, true - дефолтная ссылка (добавлена в links для визуальной наглядности)
            if ((!f_key && f_key !== false) || f_key === true)
              f_key = '__' + f_name;
            if (f_key) {
              if (!this.data[p_meta._id])
                this.data[p_meta._id] = { _meta: p_meta };
              if (!this.data[p_meta._id][f_key])
                this.data[p_meta._id][f_key] = { col: doc._meta.col, l: [] };
    ...
    async saveTransaction(config = {}) {
      ...
      for await (const doc of $db.update) {
        ...
        for await (const [key, value = {}] of Object.entries(doc)) {
          if (key.slice(0, 2) == '__') {
            // проверяем изменение связок с другими сущностями

            const initialValue =
                (meta.initial || {})[key] !== undefined ?
                  (meta.initial || {})[key] :
                  {},
              curIds = (value.l || []).join() || '',
            ...
            if (curIds != initialIds) {
              const added =
                (value.l || []).filter(
                  (id) => !initialIds.includes(id.toString())
                ) || [];
              const deleted =
                (initialValue.l || []).filter(
                  (id) => !curIds.includes(id.toString())
                ) || [];
            ...
            // проверяем остальные параметры (не связки) измененного объекта
            const initialValue =
              (meta.initial || {})[key] !== undefined ?
                (meta.initial || {})[key] :
                null;
        ...
        if (meta.initial)
          for await (const [key, value = {}] of Object.entries(meta.initial)) {
            if (doc[key] == undefined) {
              if (key.slice(0, 2) == '__') {
                // параметры-связки начинаются с "__"
      ...
      if (config.debug) {
        console.log('save debug', moment().format(), this, {
          $db,
          log_arr,
          mongo_arr,
          KEY_FIELDS: DB.KEY_FIELDS,
          KEY_LINKS: DB.KEY_LINKS,
        });
        return;
      }
      ...
      for (const [type, ...arg] of mongo_arr) {
        ...
        if (db.mongo[type]) {
          ...
          db.mongo[type](...arg).catch((e) => {
            throw e;
          });
        }
        if (
          $reverseUpdate &&
          $reverseUpdate.checkAndDeleteEmpty &&
          $reverseUpdate.checkAndDeleteEmpty()
        ) {
      ...
      if (config.log) {
        if (config.log.target._id) {
          ...
          const updateLog = [
            config.log.target.col,
            { _id: config.log.target._id },
            { $addToSet: { [config.log.path]: JSON.stringify(log_arr) } },
          ];
          db.mongo.updateOne(...updateLog).catch((e) => {
            throw e;
          });
        }
      }
