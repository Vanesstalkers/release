({
  linkPlanesByPorts({ joinPort, targetPort }) {
    ...
    joinPlane.rotate = this.getPlaneRotationByLinkedPortDirections(
      joinPort,
      targetPort,
      targetPlane.rotate
    );

    const targetLinkPoint = this.getLinkPointCoordinates(
      targetPort,
      targetPlane
    );
  ...
  getLinkPointCoordinates(port, plane) {
    ...
    const rotatedCoordinates = this.rotatePoint(
  ...
  getLinkPointFromPortDirection(port, direct, offsetSpace = 5) {
    ...
    case 'right':
      return {
        ...
        left: port.plane.width + offsetSpace,
      };
    case 'top':
      return { top: -offsetSpace, left: port.left + port.width / 2 };
    case 'bottom':
      return {
        top: port.plane.height + offsetSpace,
  ...
  getPlaneRotationByLinkedPortDirections(
    ...
    for (let i = 0; i < targetPlaneRotation; i++) {
      targetDirectWithRotate =
        this.ROTATIONS[targetDirectWithRotate].nextDirection;
    }
    let resultRotation = 0;
    while (
      this.ROTATIONS[joinDirectWithRotate].oppositeDirection !==
      targetDirectWithRotate
    ) {
