({
    ...
    dataSearch(data, query, type) { // универсальный поиск по массиву объектов
    ...
    flatten(objectOrArray, prefix = '', formatter = (k) => (k)) {
        ...
        const nestElement = (prev, value, key) => (
            lib.utils.isObjectID(value)
                ? { ...prev, ...lib.utils.flatten(value, `${prefix}${formatter(key)}`, nestedFormatter) }
    ...
    sumPropertiesOfObjects(arrayOfObjects, allowProps = []) {
        return arrayOfObjects.reduce((result, item) => {
            for (const key in item) {
                if (item.hasOwnProperty(key) && (allowProps.length === 0 || allowProps.includes(key))) {
        ...
        */
        ...
        for (const key of Object.keys(sourceObject)) {
            ...
            if (Array.isArray(tmpObject))
                tmpObject.push(tmpObjectPropValue);
            else
                tmpObject[key] = tmpObjectPropValue;
        }
    
        return {storage, proxy: new Proxy(tmpObject, {
            set: function (target, name, value, receiver) {
                ...
                lib.utils.updateStorage({target, name, value, path, storage, sourceObject});
            ...
            deleteProperty: function (target, name) {
                ...
                lib.utils.updateStorage({target, name, value: null, path, storage, sourceObject});
    ...
    updateStorage({target, name, value, path, storage, sourceObject}){
        if (Array.isArray(target)) { // для массивов не придумал ничего лучше, чем обновлять его целиком
            if (name !== 'length') {
            ...
            if (findParentKey) { // найден родитель для текущего ключа, которого нужно обновить
                /*
