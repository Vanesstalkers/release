async ({ data, msg }) => {
  ...
  const playerId =
    msg.parent == 'player' && msg.parentId ? msg.parentId : game.active_player;
  ...
  if (msg.planeId) {
    // привязка уже созданного plane
    if (
      /*!prevPlayer.ai && */ !window.checkPlayerIsActive(
        data.user.game?.player?._id,
        game
      )
    )
    ...
    const sourcePort = await data.findOne('port', {
      _id: { $in: plane.__port.l },
      code: msg.bridge[0].code.split('-')[1],
    });
    ...
    for (const zone of await data.find('zone', {
      _id: { $in: targetPlane.__zone.l },
    }))
      data.zoneIds[zone.code] = zone;
    for (const zone of await data.find('zone', {
      _id: { $in: plane.__zone.l },
    }))
      data.zoneIds[zone.code] = zone;
  } // создание нового plane
  else {
    plane = msg.plane;
    ...
    for (const [zoneCode, zone] of Object.entries(plane.zones)) {
      ...
      if (zone.dice)
        data.zoneDices[zone.t][zone.dice.join('')] = data.zoneIds[zone.code];
    }
    ...
    for (const [zoneCode, zone] of Object.entries(plane.zones)) {
      for (const [side, links] of Object.entries(zone.links)) {
        if (links.length)
          data.zoneIds[zone.code][side] = links.map(
            (link) =>
              `${data.zoneIds[planeCode + '_' + link.z]?._meta?._id}_${link.s}`
          );
      }
    }
  }
  ...
  for (const bridge of bridgeList) {
    ...
    //delete bridge.code;
    const bridgeCodeItems = (
      msg.planeId ?
        bridgeCode :
        bridgeCode.replace('NEW', plane._meta._id.toString())
    ).split('-'); // если msg.planeId нет (plane только что создан этим методом), то ссылка на создаваемый plane указывается через "NEW"
    const bridgeZone = Object.assign({}, bridge, {
      notAvailable: true,
      v1: {},
      v2: {},
      s1: [],
      s2: [],
    }); // наполняем данные для zone из конфига bridge до наполнения его мета-данными

    data.add('bridge', game, bridge);
    Object.assign(bridge, {
      link: [
        data.portList[bridgeCodeItems[0]]._meta._id,
        data.portList[bridgeCodeItems[1]]._meta._id,
      ],
    });
