async ({ data }) => {
  ...
  const dices = (
    ...
    dice.side1 = dice[0];
    dice.side2 = dice[1]; // для более наглядной работы с параметрами
    return dice;
  });
  ...
  for (const player of players) {
    ...
    if (game.config.player_hand_start) {
      // наполняем стартовую руку игрока
  ...
  for (const [planeCode, plane] of Object.entries(game.data.plane)) {
        ...
        bridge: game.data.bridge
          ...
          .map((bridge) =>
            Object.assign({}, bridge, {
              code: bridge.code.replace('-*' + planeCode, '-NEW'),
            })
          ), // ссылка на создаваемый plane указана через "NEW"
      },
    });
    ...
    if (newPlane._id) {
      ...
      game.data.bridge
        .filter((bridge) => bridge.code.includes('*' + planeCode))
        .forEach((bridge) => {
          bridge.code = bridge.code.replace(
            '*' + planeCode,
            newPlane._id.toString()
          );
        });
    }
  }
